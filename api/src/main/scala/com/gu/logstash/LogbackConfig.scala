...
import com.gu.logstash.LogStashConf
import com.typesafe.scalalogging.LazyLogging
import net.logstash.logback.layout.LogstashLayout
import org.slf4j.{LoggerFactory, Logger => SLFLogger}

class LogbackConfig(logbackOperationsPool: LogbackOperationsPool) extends LazyLogging {
  ...
  def init(config: LogStashConf): Unit = {
    if (config.enabled) {
      try {
        ...
        case ex: Throwable => logger.info(s"Error while adding Logback Kinesis appender: $ex")
      }
    } else {
